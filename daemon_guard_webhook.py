...
def push_gotify(title, message, device_name=None, webhook_url=None, priority=5):
    """推送Gotify或兼容服务，自动加设备名（自定义或VM CODE）用于区分"""
    url = webhook_url or ""
    if not url:
        return
    # 标题加上设备名
...
class DaemonApp:
    def __init__(self, master):
        self.master = master
        master.title("守护进程（日志监控+重启+VM Code提取+记忆文件夹+自定义设备名+Webhook）")
