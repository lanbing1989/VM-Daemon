...
def add_to_startup():
    ...
    try:
        ...
        winreg.SetValueEx(key, "守护进程", 0, winreg.REG_SZ, exe_path)
        winreg.CloseKey(key)
        messagebox.showinfo("设置成功", f"已设置开机自启：{exe_path}")
    except Exception as e:
        messagebox.showerror("设置失败", f"设置开机自启失败: {e}")
...
def remove_from_startup():
    ...
    try:
        import winreg
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_SET_VALUE)
        winreg.DeleteValue(key, "守护进程")
        winreg.CloseKey(key)
        messagebox.showinfo("已移除", "已移除开机自启。")
    except FileNotFoundError:
        messagebox.showinfo("未设置", "未设置开机自启。")
    except Exception as e:
...
def check_startup_status():
    ...
    try:
        ...
        value, regtype = winreg.QueryValueEx(key, "守护进程")
        winreg.CloseKey(key)
...
class DaemonApp:
    def __init__(self, master):
        ...
        # --------- 开机自启按钮区域 ---------
        autostart_frame = tk.Frame(master)
        autostart_frame.pack(pady=3)
        self.autostart_btn = tk.Button(autostart_frame, text="设置开机自启", command=self.set_autostart)
        self.autostart_btn.pack(side=tk.LEFT, padx=3)
        self.autostart_remove_btn = tk.Button(autostart_frame, text="移除开机自启", command=self.remove_autostart)
        self.autostart_remove_btn.pack(side=tk.LEFT, padx=3)
        self.autostart_status_label = tk.Label(autostart_frame, text="", fg="blue")
        self.autostart_status_label.pack(side=tk.LEFT, padx=5)
        ...
        self.no_log_action_timeout = 1800  # 30分钟=1800秒
    ...
    def start_script(self):
        try:
            ...
            if os.path.abspath(self.current_exe) == os.path.abspath(sys.executable):
                messagebox.showerror("错误", "守护进程不能守护自己！请选择正确的被控端exe。")
                return
    ...
    def monitor_logfile_only(self):
        ...
        while self.running:
            try:
                ...
                if lines:
                    self.last_log_update_time = now  # 只要有新日志就更新时间
                for line in lines:
