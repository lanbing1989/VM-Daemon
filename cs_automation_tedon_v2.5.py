...
def add_to_startup():
    if platform.system() != "Windows":
        messagebox.showwarning("提示", "当前系统不是Windows，无法设置开机自启")
        return
    try:
        exe_path = sys.executable
        import subprocess
        cmd = [
            "schtasks", "/create", "/tn", "CS自动化-灯火通明", "/tr", f'"{exe_path}"',
            "/sc", "onlogon", "/rl", "highest", "/f"
        ]
        result = subprocess.run(" ".join(cmd), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="gbk")
        if result.returncode == 0:
            messagebox.showinfo("设置成功", f"已设置任务计划开机自启（管理员权限）：{exe_path}")
        else:
            messagebox.showerror("设置失败", f"任务计划设置失败: {result.stdout}\n{result.stderr}")
    except Exception as e:
        messagebox.showerror("设置失败", f"设置任务计划开机自启失败: {e}")

def remove_from_startup():
    if platform.system() != "Windows":
        messagebox.showwarning("提示", "当前系统不是Windows，无法操作")
        return
    try:
        import subprocess
        result = subprocess.run("schtasks /delete /tn CS自动化-灯火通明 /f", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="gbk")
        if result.returncode == 0:
            messagebox.showinfo("已移除", "已移除任务计划开机自启。")
        else:
            messagebox.showerror("错误", f"移除任务计划失败: {result.stdout}\n{result.stderr}")
    except Exception as e:
...
def check_startup_status():
    ...
    try:
        import subprocess
        result = subprocess.run("schtasks /query /tn CS自动化-灯火通明", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="gbk")
        return (result.returncode == 0)
    except Exception:
        return False

def kill_steam_and_csgo_process():
    for proc in psutil.process_iter(['name']):
        try:
            name = proc.info['name']
            if name and name.lower() in ("steam.exe", "csgo.exe"):
                proc.kill()
        except Exception:
            pass

def hide_all_console_windows_loop():
...
class DaemonApp:
    def __init__(self, master):
        self.master = master
        master.title("CS自动化 - 灯火通明 v2.5")
        master.geometry("1100x560")
        ...
        # 日志区（高度调整为12行）
        self.textbox = scrolledtext.ScrolledText(master, font=("Consolas", 11), width=120, height=12)
        self.textbox.grid(row=0, column=0, columnspan=3, padx=12, pady=(10,0), sticky="nsew")

        config_frame = tk.LabelFrame(master, text="配置", font=("微软雅黑", 11, "bold"))
        config_frame.grid(row=1, column=0, columnspan=3, sticky="ew", padx=16, pady=8, ipadx=3, ipady=3)

        tk.Label(config_frame, text="文件夹路径:", font=("微软雅黑", 10)).grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.folder_var = tk.StringVar()
        self.folder_entry = tk.Entry(config_frame, textvariable=self.folder_var, width=60, font=("Consolas", 10))
        self.folder_entry.grid(row=0, column=1, sticky="ew", padx=5, pady=5)
        self.browse_btn = tk.Button(config_frame, text="选择文件夹", command=self.choose_folder, width=12)
        self.browse_btn.grid(row=0, column=2, padx=5, pady=5)
        config_frame.grid_columnconfigure(1, weight=1)

        tk.Label(config_frame, text="设备名称:", font=("微软雅黑", 10)).grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.device_name_var = tk.StringVar()
        self.device_name_entry = tk.Entry(config_frame, textvariable=self.device_name_var, width=60, font=("Consolas", 10))
        self.device_name_entry.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        self.devname_set_btn = tk.Button(config_frame, text="设置", command=self.set_device_name, width=12)
        self.devname_set_btn.grid(row=1, column=2, padx=5, pady=5)

        tk.Label(config_frame, text="Webhook URL:", font=("微软雅黑", 10)).grid(row=2, column=0, sticky="e", padx=5, pady=5)
        self.webhook_var = tk.StringVar()
        self.webhook_entry = tk.Entry(config_frame, textvariable=self.webhook_var, width=60, font=("Consolas", 10))
        self.webhook_entry.grid(row=2, column=1, sticky="ew", padx=5, pady=5)
        self.webhook_set_btn = tk.Button(config_frame, text="设置", command=self.set_webhook_url, width=12)
        self.webhook_set_btn.grid(row=2, column=2, padx=5, pady=5)

        autostart_frame = tk.Frame(config_frame)
        autostart_frame.grid(row=3, column=0, columnspan=3, sticky="w", pady=5)
        self.autostart_btn = tk.Button(autostart_frame, text="设置开机自启", command=self.set_autostart, width=16)
        self.autostart_btn.pack(side=tk.LEFT, padx=4)
        self.autostart_remove_btn = tk.Button(autostart_frame, text="移除开机自启", command=self.remove_autostart, width=16)
        self.autostart_remove_btn.pack(side=tk.LEFT, padx=4)
        self.autostart_status_label = tk.Label(autostart_frame, text="", fg="red", font=("微软雅黑", 10, "bold"))
        self.autostart_status_label.pack(side=tk.LEFT, padx=8)
        self.update_autostart_status()

        button_frame = tk.Frame(master)
        button_frame.grid(row=2, column=0, columnspan=3, sticky="ew", padx=0, pady=10)
        self.start_btn = tk.Button(button_frame, text="启动", command=self.start_script, width=12, font=("微软雅黑", 11))
        self.start_btn.pack(side=tk.LEFT, padx=8)
        self.restart_btn = tk.Button(button_frame, text="重启", command=self.restart_script, width=12, font=("微软雅黑", 11))
        self.restart_btn.pack(side=tk.LEFT, padx=8)
        self.stop_btn = tk.Button(button_frame, text="停止", command=self.stop_script, width=12, font=("微软雅黑", 11))
        self.stop_btn.pack(side=tk.LEFT, padx=8)
        self.net_diag_btn = tk.Button(button_frame, text="网络检测", command=self.threaded_manual_network_diagnose, width=12, font=("微软雅黑", 11))
        self.net_diag_btn.pack(side=tk.LEFT, padx=8)

        status_frame = tk.Frame(master)
        status_frame.grid(row=3, column=0, sticky="w", padx=22, pady=2)
        self.status_label = tk.Label(status_frame, text="状态: 未启动", font=("微软雅黑", 11))
        self.status_label.pack(side=tk.LEFT)
        self.last_detect_label = tk.Label(status_frame, text="最近检测：无", fg="green", font=("微软雅黑", 11))
        self.last_detect_label.pack(side=tk.LEFT, padx=24)

        vmcode_frame = tk.Frame(master)
        vmcode_frame.grid(row=3, column=2, sticky="e", padx=22, pady=2)
        tk.Label(vmcode_frame, text="VM Code:", font=("Consolas", 14)).pack(side=tk.LEFT)
        self.vm_code_var = tk.StringVar()
        self.vm_code_label = tk.Label(vmcode_frame, textvariable=self.vm_code_var, font=("Consolas", 16, "bold"), fg="blue")
        self.vm_code_label.pack(side=tk.LEFT, padx=8)
        self.copy_code_btn = tk.Button(vmcode_frame, text="复制", command=self.copy_vm_code, font=("微软雅黑", 10))
        self.copy_code_btn.pack(side=tk.LEFT, padx=5)

        copyright_frame = tk.Frame(master)
        copyright_frame.grid(row=4, column=2, sticky="e", padx=10, pady=(1,6))
        ...
        self.last_log_update_time = None
        self.no_log_action_timeout = 1800
        self.restart_count = 0

        self.load_config_folder()
        self.load_device_name()
        self.load_webhook_url()
        self.master.after(1000, self.try_auto_start_script)
    ...
    def update_autostart_status(self):
        if check_startup_status():
            self.autostart_status_label.config(text="已设为开机自启", fg="green")
        else:
            self.autostart_status_label.config(text="未设为开机自启", fg="red")

    def try_auto_start_script(self):
        if self.folder:
            self.start_script()
    ...
    def start_script(self):
        try:
            ...
            if not self.current_exe:
                ...
                return
            if os.path.abspath(self.current_exe) == os.path.abspath(sys.executable):
                messagebox.showerror("错误", "本程序不能守护自己！请选择正确的被控端exe。")
                return
            ...
            if os.path.exists(self.logfile_path):
                ...
                self.logfile_pos = 0
            self.running = True
            self.textbox.insert(tk.END, f"启动（隐藏窗口）：{os.path.basename(self.current_exe)}\n")
            try:
                ...
                return
            self.status_label.config(text=f"状态: 运行中（入口：{os.path.basename(self.current_exe)}，窗口已隐藏）")
            self.fail_times.clear()
            self.just_restarted = False
            self.stopplayer_done_time = None
            self.last_log_update_time = time.time()
            # **只保留日志文件监控线程，不再监听 stdout/stderr**
            self.monitor_thread = threading.Thread(target=self.monitor_logfile_only, daemon=True)
            self.monitor_thread.start()
            threading.Thread(target=hide_all_console_windows_loop, daemon=True).start()
        except Exception as e:
    ...
    def monitor_logfile_only(self):
        ...
        if self.last_log_update_time is None:
            self.last_log_update_time = time.time()
        while self.running:
            try:
                time.sleep(1)
                now = time.time()
                if self.last_log_update_time is not None and (now - self.last_log_update_time >= self.no_log_action_timeout):
                    self.textbox.after(0, lambda: self.textbox.insert(tk.END,
                        "\n[WatchDog] 检测到日志30分钟无新内容，自动重启\n"))
                    push_gotify("日志无动作自动重启", "日志30分钟无新内容，自动重启。", self.get_device_name(), self.webhook_var.get().strip(), 6)
                    self.restart_script()
                    return
                ...
                for line in lines:
                    ...
                    if is_connection_loss_error(line):
                        ...
                        push_gotify("掉线自动重连成功", f"程序检测到掉线，已自动重启并恢复连接。\nVM CODE: {current_vm_code}", device_name=device_name, webhook_url=webhook_url, priority=5)
...
if __name__ == "__main__":
    if not check_single_instance():
        tk.Tk().withdraw()
        messagebox.showerror("多开检测", "已有CS自动化在运行，禁止多开。")
        sys.exit(0)
    root = tk.Tk()
    app = DaemonApp(root)
